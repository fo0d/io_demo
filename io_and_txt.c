...
void show_lines(struct y3_list *list);
int proc_error(int n_args, ...);
void stream_info(int n_args, ...);
    ...
    for(i = 0; i < n_args; i++) {
        ...
        if(stream) {
            ...
            for(j = 0; j < stream->nlinks; j++) {
                ...
                if(test)
                    fprintf(stderr,"linked to: stream id [%d] name [%s]\n", test->id,
                            (test->name != (char *)0) ? test->name : "<no_name>");
            }
            fprintf(stderr,"redirect flag is %s\n", stream->redirect > 0 ? "ON" : "OFF");
            fprintf(stderr,"redirection lock is %s\n", stream->redirect > 0 ? "ON" : "OFF");
...
proc_error(int n_args, ...)
{
    ...
    for(i = 0; i < n_args; i++) {
        ...
        if(!stream) {
            ...
            fprintf(stderr,"stream handle [%d] io error: %s\n", stream->handle, stream->error);
            return 1;
        }
    }
    return 0;
...
main(void)
{
    ...
    char *my_msg = "Testing linking, this output is going to stdout and output.txt\n";
    ...
    out = y3_io_new("output.txt", open_flags_osx_new | _O_CREAT, _S_IWRITE | _S_IREAD, -1);
    out2 = y3_io_new("output2.txt", open_flags_osx_new | _O_CREAT, _S_IWRITE | _S_IREAD, -1);
    ...
    fprintf(stderr,"%lx bytes written to file %s\n", in->bytes_read, out->name);
    ...
    //
    // the input shouldn't goto file.txt since it's opened for reading only, and the
    // error will be set.
    //
...
show_lines(struct y3_list *txt)
{
    ...
    for( ; txt; txt = txt->next) {
        ...
        fprintf(stderr, "line[%lx] in file[%s] : %s", li->id_real + 1, li->fname, li->txt);
    }
